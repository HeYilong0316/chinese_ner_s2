...
class KeywordProcessor(object):
    ...
    def get_all_keywords(self, term_so_far='', current_dict=None):
        ...
        if current_dict is None:
            current_dict = self.keyword_trie_dict
        for tmp_key_word in current_dict:
            if tmp_key_word == '_keyword_':
                terms_present[term_so_far] = current_dict[tmp_key_word]
            else:
                sub_values = self.get_all_keywords(term_so_far + tmp_key_word, current_dict[tmp_key_word])
                for tmp_key_word in sub_values:
                    terms_present[tmp_key_word] = sub_values[tmp_key_word]
