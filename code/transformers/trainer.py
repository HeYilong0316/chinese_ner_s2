...
from .optimization import AdamW, get_linear_schedule_with_warmup, get_constant_schedule_with_warmup, get_polynomial_decay_schedule_with_warmup
...
class Trainer:
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int, num_warmup_steps: int):
        ...
        if self.optimizer is None:
            ...
            # for layer_model in self.model.bert.encoder.layer[::-1]:
            #     layer_grouped_parameters = list(layer_model.named_parameters())

            #     optimizer_grouped_parameters += [
            #         {
            #         'params': [p for n, p in layer_grouped_parameters if not any(nd in n for nd in no_decay)],
            #         'weight_decay': self.args.weight_decay,
            #         'lr': lr
            #         },
            #         {
            #         'params': [p for n, p in layer_grouped_parameters if any(nd in n for nd in no_decay)],
            #         'weight_decay': self.args.weight_decay,
            #         'lr': lr
            #         },
            #     ]
            #     lr = lr * self.args.layer_decay
            

            bert_param_optimizer = list(self.model.bert.named_parameters())
            # layer_grouped_parameters = [id(p) for n, p in list(self.model.bert.encoder.layer.named_parameters())]
            # bert_param_optimizer = [(n, p) for n, p in bert_param_optimizer if (id(p) not in layer_grouped_parameters)]
            ...
            optimizer_grouped_parameters += [
                ...
                },
                {
                    ...
                    'weight_decay': self.args.linear_weight_decay,
            ...
            if self.model.layer_fusion is not None:
                layer_weight_param_optimizer = list(self.model.layer_fusion.named_parameters())
                optimizer_grouped_parameters += [
                    {
                        'params': [p for n, p in layer_weight_param_optimizer if not any(nd in n for nd in no_decay)], 
                        'weight_decay': self.args.layer_weight_weight_decay,
                        'lr': self.args.layer_weight_learning_rate
                    },
                    {
                        'params': [p for n, p in layer_weight_param_optimizer if any(nd in n for nd in no_decay)], 
                        'weight_decay': 0.0,
                        'lr': self.args.layer_weight_learning_rate
                    }
            ]
            ...
            if self.model.lstm:
                lstm_param_optimizer = list(self.model.lstm.named_parameters())
                optimizer_grouped_parameters += [
                    {
                        'params': [p for n, p in lstm_param_optimizer if not any(nd in n for nd in no_decay)], 
                        'weight_decay': self.args.lstm_weight_decay,
            ...
            if self.model.id_cnn:
                ...
                optimizer_grouped_parameters += [
                    {
                        'params': [p for n, p in idcnn_param_optimizer if not any(nd in n for nd in no_decay)], 
                        'weight_decay': self.args.idcnn_weight_decay,
                        'lr': self.args.idcnn_learning_rate
                    },
                    {
                        'params': [p for n, p in idcnn_param_optimizer if any(nd in n for nd in no_decay)], 
                        'weight_decay': 0.0,
                        'lr': self.args.idcnn_learning_rate
                    }
            ]
                ...
                optimizer_grouped_parameters += [
                    {
                        ...
                        'weight_decay': self.args.crf_weight_decay,
        ...
        if self.lr_scheduler is None:
            ...
            # )
            # self.lr_scheduler = get_constant_schedule_with_warmup(self.optimizer, num_warmup_steps=self.args.warmup_steps)
    ...
    def setup_wandb(self):
        ...
        if self.is_world_process_zero():
            ...
            try:
                combined_dict = {
                    **self.model.config.to_dict(), **self.args.to_sanitized_dict()}
    ...
    def setup_comet(self):
        ...
        if self.is_world_master():
            ...
            if experiment is not None:
                experiment._set_model_graph(
                    self.model, framework="transformers")
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                # print(step+1, len(epoch_iterator))
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if self.args.fp16 and _use_native_amp:
                        ...
                        torch.nn.utils.clip_grad_norm_(amp.master_params(
                            self.optimizer), self.args.max_grad_norm)
                    else:
                        torch.nn.utils.clip_grad_norm_(
                            model.parameters(), self.args.max_grad_norm)
    ...
    def log(self, logs: Dict[str, float], iterator: Optional[tqdm] = None) -> None:
        ...
        if self.total_flos is not None:
            if self.args.local_rank != -1:
                total_flos = distributed_broadcast_scalars(
                    [self.total_flos]).sum().item()
            else:
            ...
            if self.is_world_process_zero():
                ...
                if experiment is not None:
                    experiment._log_metrics(
                        logs, step=self.global_step, epoch=self.epoch, framework="transformers")
    ...
    def is_local_master(self) -> bool:
        ...
        """
        warnings.warn(
            "This method is deprecated, use `Trainer.is_local_process_zero()` instead.", FutureWarning)
    ...
    def is_world_master(self) -> bool:
        ...
        """
        warnings.warn(
            "This method is deprecated, use `Trainer.is_world_process_zero()` instead.", FutureWarning)
    ...
    def _sorted_checkpoints(self, checkpoint_prefix=PREFIX_CHECKPOINT_DIR, use_mtime=False) -> List[str]:
        ...
        glob_checkpoints = [str(x) for x in Path(
            self.args.output_dir).glob(f"{checkpoint_prefix}-*")]
    ...
    def predict(self, test_dataset: Dataset, description="Prediction") -> PredictionOutput:
        """
        ...
        return self.prediction_loop(test_dataloader, description=description)
        ...
        if self.args.local_rank != -1:
            ...
            if label_ids is not None:
                label_ids = nested_xla_mesh_reduce(
                    "eval_label_ids", label_ids, torch.cat)
    ...
    def prediction_step(
        ...
        has_labels = all(inputs.get(
            k) is not None for k in self.args.label_names)
        inputs = self._prepare_inputs(inputs)
        ...
        if has_labels:
            labels = tuple(inputs.get(name).detach()
                           for name in self.args.label_names)
            if len(labels) == 1:
                labels = labels[0]
        else:
            labels = None
